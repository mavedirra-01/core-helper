...
class Nessus:
	...
	def get_auth(self, verbose=True):
		...
		with log.progress("Retrieving api tokens") as p:
			try:
	...
	def exclude_targets(self):
		with log.progress("Adding targets to reject list") as p:
			try:
	...
	def update_settings(self):
		with log.progress("Updating settings") as p:
	...
	def import_policies(self):
		with log.progress("Importing policies") as p:
			try: 
	...
	def create_scan(self, launch):
		with log.progress("Creating new scan") as p:
	...
	def scan_action(self, action):
		with log.progress(f"Sending {action} request to \"{self.project_name}\"") as p:
			try:
	...
	def monitor_scan(self):
		...
		time_elapsed = 0
		with log.progress(f"Scan status") as p:
	...
	def export_scan(self):
		try:
			...
			for k,v in formats.items():
				with log.progress(f"Exporting {k} file") as p:
					# get scan token
					data = v
	...
	def analyze_results(self, scan_file):
		with log.progress("Analyzing results") as p:
			try:
...
if __name__ == "__main__":
	...
	if args.mode == "analyze":
		if not args.scan_file:
			log.failure("You must provide a scan file (-s)")
			exit()
		username, password = get_creds()

	else:
		if args.drone is None or args.client is None:
			log.failure("You must provide the drone name (-d) and the client name (-c)")
			exit()
		if args.mode == "deploy" or args.mode == "trigger":
			if not args.policy or not args.targets:
				log.failure("You must provide a policy file (-p) and a targets file (-t) for this mode")
				exit()
		# Check drone name/ip and create url (for nessus) 
		if "http" in args.drone:
			log.failure("You must provide only the drone name/dns, or IP. Do not use http link")
			exit()
		username, password = get_creds()
